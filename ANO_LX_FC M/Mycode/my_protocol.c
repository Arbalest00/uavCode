...
void pi_receive(u8 data) // ��ݮ�ɽ���Э�� ����2
{
	...
	else if (state_1 == 5) // yaw�ƶ�ָ��
	{
		state_1 = 6;
		RxBuffer[5] = data;
	}
	else if (state_1 == 6) // �׶��л�ָ��
	{
		RxBuffer[6] = data;
		state_1 = 7;
	}
	else if (state_1 == 7) // �׶��л�ָ��
	{
		RxBuffer[7] = data;
		state_1 = 8;
	}
	else if (state_1 == 8 && data == 0xFF) // ������� ֡β0xFF
	{
		RxBuffer[8] = data;
		received_data.sp_side = RxBuffer[7];
		received_data.task_sta = RxBuffer[1]; // ����ָʾ
...
void lidar_receive(u8 data) // �����״�LDS-08���� ����3//�ѷ���
{
...
void lidar_cal(struct lidar_data *lidar) // lidar�������ݼ���//�ѷ���
{
...
void PID_init()
{
	...
	height_PID.err_sum = 0;
	height_PID.last_out = 0;
}
s16 height_set(u32 height, u16 height_set)
{
	s16 output_increment = 0; // 这是输出的增量
	s16 output = 0;			  // 这是最终的输出值
	...
	// 计算输出增量
	output_increment = height_PID.p * (height_PID.err_current - height_PID.err_last) +
					   height_PID.i * height_PID.err_current +
	...
	// 更新输出值（增量式更新）
	output = height_PID.last_out + output_increment;
...
/* s16 height_set(u32 height, u16 height_set)
{
	...
	height_PID.target = height_set;
	height_PID.err_current = height_PID.target - height_PID.actual;
	output = height_PID.p * (height_PID.err_current - height_PID.err_last) + height_PID.iheight_PID.err_current + height_PID.d(height_PID.err_current - 2 * height_PID.err_last + height_PID.err_previous);
	height_PID.err_previous = height_PID.err_last;
	height_PID.err_last = height_PID.err_current;
	if (output > 30)
		output = 30;
